-- ============================================================
-- LookaGenius — Full Supabase Schema (fixed order)
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    role TEXT NOT NULL CHECK (role IN ('admin', 'teacher', 'student', 'parent', 'engineer', 'accountant')) DEFAULT 'student',
...
CREATE TABLE IF NOT EXISTS public.certificates (
    ...
    verification_token TEXT UNIQUE DEFAULT md5(random()::text || clock_timestamp()::text),
...
-- 14. VIDEO ACCESS TOKENS
CREATE TABLE IF NOT EXISTS public.video_access (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES public.profiles(id) NOT NULL,
    lesson_id UUID REFERENCES public.lessons(id) ON DELETE CASCADE NOT NULL,
    access_token TEXT UNIQUE DEFAULT md5(random()::text || clock_timestamp()::text || random()::text),
...
CREATE POLICY "courses_select_enrolled" ON public.courses FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.enrollments WHERE enrollments.course_id = courses.id AND enrollments.student_id = auth.uid()));
...
CREATE POLICY "modules_select_published" ON public.modules FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = modules.course_id AND courses.is_published = true AND courses.is_approved = true));
CREATE POLICY "modules_select_enrolled" ON public.modules FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.enrollments WHERE enrollments.course_id = modules.course_id AND enrollments.student_id = auth.uid()));
...
CREATE POLICY "modules_insert_teacher" ON public.modules FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = modules.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "lessons_select_published" ON public.lessons FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.modules JOIN public.courses ON courses.id = modules.course_id WHERE modules.id = lessons.module_id AND courses.is_published = true AND courses.is_approved = true));
CREATE POLICY "lessons_select_enrolled" ON public.lessons FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.modules JOIN public.enrollments ON enrollments.course_id = modules.course_id WHERE modules.id = lessons.module_id AND enrollments.student_id = auth.uid()));
CREATE POLICY "lessons_select_own" ON public.lessons FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.modules JOIN public.courses ON courses.id = modules.course_id WHERE modules.id = lessons.module_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "lessons_insert_teacher" ON public.lessons FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM public.modules JOIN public.courses ON courses.id = modules.course_id WHERE modules.id = lessons.module_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "enrollments_select_teacher" ON public.enrollments FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = enrollments.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "lp_select_teacher" ON public.lesson_progress FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = lesson_progress.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "assessments_select_published" ON public.assessments FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = assessments.course_id AND courses.is_published = true AND courses.is_approved = true) AND is_published = true);
CREATE POLICY "assessments_select_enrolled" ON public.assessments FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.enrollments WHERE enrollments.course_id = assessments.course_id AND enrollments.student_id = auth.uid()));
CREATE POLICY "assessments_select_own" ON public.assessments FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = assessments.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "assessments_insert_teacher" ON public.assessments FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = assessments.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "questions_select_enrolled" ON public.questions FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.assessments JOIN public.enrollments ON enrollments.course_id = assessments.course_id WHERE assessments.id = questions.assessment_id AND enrollments.student_id = auth.uid()));
CREATE POLICY "questions_select_teacher" ON public.questions FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.assessments JOIN public.courses ON courses.id = assessments.course_id WHERE assessments.id = questions.assessment_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "questions_insert_teacher" ON public.questions FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM public.assessments JOIN public.courses ON courses.id = assessments.course_id WHERE assessments.id = questions.assessment_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "attempts_select_teacher" ON public.attempts FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.assessments JOIN public.courses ON courses.id = assessments.course_id WHERE assessments.id = attempts.assessment_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "certificates_select_teacher" ON public.certificates FOR SELECT
    USING (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = certificates.course_id AND courses.instructor_id = auth.uid()));
...
CREATE POLICY "certificates_insert_teacher" ON public.certificates FOR INSERT
    WITH CHECK (EXISTS (SELECT 1 FROM public.courses WHERE courses.id = certificates.course_id AND courses.instructor_id = auth.uid()));
...
CREATE INDEX IF NOT EXISTS idx_courses_published ON public.courses(is_published, is_approved);
...
CREATE INDEX IF NOT EXISTS idx_revenues_teacher ON public.revenues(teacher_id);
