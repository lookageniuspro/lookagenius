/**
 * db.js
 * Central Database - localStorage Mock with Supabase sync
 */
...
function __syncToSupabase(data) {
    if (window.__supabase && window.__supabase.isReady) {
        window.__supabase.pushAll(data);
    }
}

async function __syncUserToSupabase(user) {
    if (!window.__supabase || !window.__supabase.isReady) return;
    try {
        const client = window.__supabase.client;
        ...
        const { error } = await client.from('users').upsert({
            ...
            country_code: user.countryCode || '+20',
            parent_phone: user.parentPhone || '',
            education_stage: user.educationStage || '',
...
window.db = {
    ...
    addUser: (user) => {
        ...
        // Sync user to Supabase auth-compatible table
        __syncUserToSupabase(user);
        return user;
    },
    ...
    updateUser: (id, updates) => {
        ...
        window.db.saveData(data);
        __syncUserToSupabase(data.users[index]);
        return true;
    },
